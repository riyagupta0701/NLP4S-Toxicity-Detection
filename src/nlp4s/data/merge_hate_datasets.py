...
# Binary: only "normal" (single label) → 0, everything else → 1.
# Compound labels like "offensive_normal" are treated as 1 (per the paper).
...
for lang, splits in HASOC_FILES.items():
    for split_name, url in splits.items():
        ...
        if url.endswith(".xlsx"):
            ...
            df = pd.read_csv(url)
        # Columns are: tweet_id, text, task1, task2, ID
        df["language"] = lang
        df["split"] = split_name
...
# 7.  Save
# ─────────────────────────────────────────────────────────────────────────────
train_df.to_csv("/Users/alexflorea/Desktop/Delft_Programming/Y1/Q4/Inclusive Speech Tech/NLP4S-Toxicity-Detection/data/processed/train_split/hate_train.csv", index=False)
val_df.to_csv("/Users/alexflorea/Desktop/Delft_Programming/Y1/Q4/Inclusive Speech Tech/NLP4S-Toxicity-Detection/data/processed/train_split/hate_val.csv", index=False)
